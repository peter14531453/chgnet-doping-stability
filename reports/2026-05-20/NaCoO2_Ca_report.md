...
This study investigates whether **Ca** (the *dopant*) can be stably inserted into the crystal structure of **NaCoO2** (the *host material*) by replacing one of the **Na** atoms already present in the crystal. This process is called *doping*.
...
An MSD slope this close to zero (-0.00169 Å²/ps) means the dopant was essentially locked in place — it barely moved at all during the simulation. The farthest the dopant moved from its starting position was **0.69 Å** (about 1.3 times the Bohr radius), with a final mean-squared displacement of **0.078 Å²**.

| Metric | Value | Threshold | Result |
|--------|-------|-----------|--------|
| MSD slope | **-0.00169 Å²/ps** | < 0.005 Å²/ps | ✅ PASS |
| Max displacement | **0.693 Å** | reference only | — |
| Final MSD | **0.0782 Å²** | reference only | — |

**Local bonding environment during MD:**

The coordination number ranged from **4 to 7** (average: 5.7), deviating significantly from the relaxed value of 6. This indicates the local structure around the dopant changed during heating, which may signal structural distortion. The average dopant–oxygen bond length during the simulation was **2.286 Å**.

| Metric | Value | Threshold | Result |
|--------|-------|-----------|--------|
| Coordination range | **4–7** (mean 5.7) | within ±1 of 6 | ❌ FAIL |
| Avg bond length (MD) | **2.286 Å** | reference only | — |
...
*Host: NaCoO2 | Dopant: Ca | Temperature: 250°C | Sites tested: 1*